...
class AuctionProblem:
    ...
    def __init__(self, auction, bidders):
        ...
        item_counts = np.array(item_counts, dtype=int)
        ...
        self._bidder_strides = np.array(range(nBidders), dtype=int) * nCases
    ...
    def __str__(self):
        ...
        for b,bidder in enumerate(self.bidders):
            ...
            nPages = np.prod(shape_c, dtype=int)
...
class AuctionSolver:
    ...
    def solve(self, verbose=False):
        ...
        starting_alloc = np.zeros((self._nBidders,self._nItems), dtype=int)
...
def run_auction(file, verbose=0):
    ...
    with open('auction_results.txt', 'w') as f:
